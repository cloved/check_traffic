...
#
# -k	i/o
#	checking for in or out traffic only
...
print_full_help_msg(){
	$Echo "Usage:"
	$Echo "$0 [ -v ] [ -6 ] [ -i Suffix ] [ -F s|S ] [-p N] [ -r ] -V 1|2c|3 ( -C snmp-community | -A \"AuthString\" (when use snmp v3, U must give the AuthString)) -H host [ -L ] -I interface -k i -w in-warning-value  -c in-critical-value -K/M -B/b "
	$Echo "Example:"
	$Echo "${0} -V 2c -C public -H 127.0.0.1 -I 4 -k i -w 200 -c 300 -K -B"
	$Echo "Or -r to use Range Value Options:"
	$Echo "${0} -V 2c -C public -H 127.0.0.1 -I 4 -k i  -r -w 200-300 -c 100-400 -K -B"
	$Echo "Or -p N to use Traffic Jitter Options:"
	$Echo "${0} -V 2c -C public -H 127.0.0.1 -I 4 -k i -p 2 -w 45 -c 55"
	...
	$Echo "${0} -V 3 -A \"-u kschmidt -l authPriv -a MD5 -A mysecretpass -x DES -X mypassphrase\" -H 127.0.0.1 -I 4 -k i -w 200 -c 300 -K -B"
}
...
if [ $# -lt 1 ]; then
	...
	while getopts :v6rhi:p:F:V:C:A:H:I:k:LKMBbw:c: OPTION
	do
		case $OPTION
			in
			v)
...
if [ "$kDirection" != "i" -a "$kDirection" != "o" ]; then
	$Echo "Args Error."
	print_full_help_msg
	exit 3
fi
...
if [ -z "$Interface" -o -z "$kDirection" -o -z "$WarningV" -o -z "$CriticalV" ] ; then
	$Echo "Args Error."
	print_full_help_msg
	exit 3
fi
...
if [ $UseRange = "True" ] ;then
	...
	if [ $WVC -ne 1 -o $CVC -ne 1 ] ; then
		...
		if [ $check_1b -ne 1 -o $check_1e -ne 1 ] ; then
...
CF_HIST_DATA="/var/tmp/check_traffic_v127_ev_${kDirection}_${Host}_${Interface}.hist_dat_${USER}_${Suffix}"
...
if [ $UseRange = "True" ] ;then
	...
	if [ $check_w1b -eq 1 -a $check_w1e -eq 1 ] ; then
		Severity="0";
		Msg="OK";
		to_debug Severity is $Severity , Msg is $Msg 
	elif [ $check_c1b -eq 1 -a $check_c1e -eq 1 ] ; then
		Severity="1";
		Msg="Warning";
		to_debug Severity is $Severity , Msg is $Msg 
	else
	...
	if [ $check_w1 -eq 1 ] ; then
		Severity="0";
		Msg="OK";
		to_debug Severity is $Severity , Msg is $Msg 
	elif [ $check_c1 -eq 1  ] ; then
		Severity="1";
		Msg="Warning";
		to_debug Severity is $Severity , Msg is $Msg 
	else
...
if [ $UseRange = "True" ] ;then
	...
	if [ $Format"AA" = "SAA" ]; then
		$Echo "$Msg" "-" $kDirecitonMsg "$In"${Unit_1}${Unit_2}|${kDirectionMsg}\=${In}${Unit_1}${Unit_2}\;\;\;0\;0
	elif [ $Format"AA" = "sAA" ]; then
		$Echo "$Msg" "-" ${kDirectionMsg}/Interval "$In"${Unit_1}${Unit_2}/"$Interval"s \|${kDirectionMsg}\=${In}${Unit_1}${Unit_2}\;\;\;0\;0 Interval\=${Interval}s\;1200\;1800\;0\;0 
	else
		$Echo "$Msg" "-" The Traffic $kDirectionMsg is "$In"${Unit_1}${Unit_2}. The Check Interval is "$Interval"s \|${kDirectionMsg}\=${In}${Unit_1}${Unit_2}\;\;\;0\;0 Interval\=${Interval}s\;1200\;1800\;0\;0 
	fi
	exit $Severity
	...
	if [ $Format"AA" = "SAA" ]; then
		$Echo "$Msg" "-" Traffic Jitter $kDirectionMsg "$DiffRIAVG""%"/"$DiffROAVG""%"\|${kDirectionMsg}\=$DiffRIAVG\;${W1}\;${C1}\;0\;0 
	elif [ $Format"AA" = "sAA" ]; then
		$Echo "$Msg" "-" Traffic Jitter $kDirectionMsg/Interval "$DiffRIAVG""%"/"$DiffROAVG""%"/"$DiffAVGTotal""%"/"$Interval"s \|${kDirectionMsg}\=$DiffRIAVG\;${W1}\;${C1}\;0\;0 Interval\=${Interval}s\;1200\;1800\;0\;0 
	else
		$Echo "$Msg" "-" The Traffic Jitter $kDirectionMsg is "$DiffRIAVG""%". The Check Interval is "$Interval"s \|${kDirectionMsg}\=$DiffRIAVG\;${W1}\;${C1}\;0\;0 Interval\=${Interval}s\;1200\;1800\;0\;0 
	fi
	exit $Severity
	...
	if [ $Format"AA" = "SAA" ]; then
		$Echo "$Msg" "-" $kDirectionMsg "$In"${Unit_1}${Unit_2}|${kDirectionMsg}\=${In}${Unit_1}${Unit_2}\;${W1}\;${C1}\;0\;0
	elif [ $Format"AA" = "sAA" ]; then
		$Echo "$Msg" "-" $kDirection/Interval "$In"${Unit_1}${Unit_2}/"$Interval"s \|${kDirectionMsg}\=${In}${Unit_1}${Unit_2}\;${W1}\;${C1}\;0\;0 Interval\=${Interval}s\;1200\;1800\;0\;0 
	else
		$Echo "$Msg" "-" The Traffic $kDirectionMsg is "$In"${Unit_1}${Unit_2}. The Check Interval is "$Interval"s \|${kDirectionMsg}\=${In}${Unit_1}${Unit_2}\;${W1}\;${C1}\;0\;0 Interval\=${Interval}s\;1200\;1800\;0\;0 
	fi
	exit $Severity
fi
